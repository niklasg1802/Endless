...
from pipeline import comfy, openrouter  # noqa: E402
...
def build_keyframe_prompt(scene, shot, sheets):
    """Prompt for the image model. Character sheets are passed as input images."""
    parts = []
    if scene["style_block"]:
        parts.append(scene["style_block"])
    parts.append(shot["keyframe_prompt"])
    if sheets:
        # The image model receives the sheets in order, so refer to them by index.
        who = [n for n in (shot.get("characters") or [])
               if scene["characters"].get(n)]
        refs = ", ".join(f"<Picture {i}> is {n}" for i, n in enumerate(who, start=1))
        parts.append(
            f"Match the character designs exactly as shown in the reference images "
            f"({refs}). Keep the same proportions, colours and features."
        )
...
def cmd_sheets(scene, force=False):
    ...
    for key, ch in scene["characters"].items():
        ...
        images, usage = openrouter.generate_image(prompt)
        out.write_bytes(images[0])
        print(f"    -> sheets/{key}.png  cost={usage.get('cost')}")
        made += 1
...
def cmd_plan(scene):
    ...
    print(f"\n  keyframes: 1 image model call per shot (~$0.07 each)")
    print(f"  render:    {len(scene['shots'])} local H3 clip(s) on the 3090 Ti")
    return 0
...
def cmd_keyframes(scene, only=None, force=False):
    ...
    for shot in scene["shots"]:
        ...
        print(f"  keyframe {shot['id']} (sheets={len(sheets)}) ...", flush=True)
        images, usage = openrouter.generate_image(prompt, images=sheets)
        out.write_bytes(images[0])
        print(f"    -> {out.relative_to(d.parent)}  cost={usage.get('cost')}")
        done += 1
